...
describe('Show cluster', () => {
  ...
  it('If Scheduler Cluster and Seed Peer Cluster exist, deleting the cluster fails', () => {
    ...
    cy.get('.css-1rqacr6-MuiButtonBase-root-MuiButton-root').click();
    cy.get('#deleteCluster').click();
    cy.get('.MuiAlert-message').should('be.visible').and('contain', 'scheduler cluster exists scheduler');
  });
  ...
  it('Delete scheduler', () => {
    ...
    cy.get('.MuiAlert-message').should('be.visible').and('contain', 'Submission successful!');
    cy.wait(1000);
    cy.get(
